...
async function tickEngine() {
    ...
    function animate() {
        ...
        if (elapsed > fpsInterval) {
            ...
            window.renderer.drawSprites(debugSprites.concat(sprites).concat(uiSprites).concat(debugSprites));
        }
    }
    startAnimating(60);
}
...
function getAngleFromCollision(spr, spr2, checkcollide) {
    if (true) {
        ...
        var right = 0;
        roundCords(spr);
        while ((check < 900 && checkcollide(spr.x, spr.y, spr2)) || force) {
            right += 1;
            force = false;
            spr.x = possave.x;
            spr.y = possave.y;
            moveSteps(spr.speedcap, spr);
            roundCords(spr);
            spr.direction -= 1;
            check += 1;
        ...
        var outAngle = scr_wrap_angle((Math.round(ScratchMod(angle, 361) / 5) * 5) - 90) + 90;
        ...
        return Math.round((scr_wrap_angle(a - 90) + 90) / 2) * 2;
    } else {
        return 90;
    }
}
async function stickToFloor(spr, spr2, checkcollide) {
    if (spr.onfloor) {
        var check = 0;
        var checkamout = Math.abs(spr.speed) + 20;
        var felloff = false;
        ...
        if (felloff) {
            ...
            var ang = Math.round(ScratchMod(spr.direction, 360) / 90) * 90;
            if (ang == 0 || ang == 180) {
                spr.gravity =
                    Math.abs(moveStepsSpeed(Math.abs(spr.speed), spr.direction)[1] * 0.5);
                spr.speed =
                    moveStepsSpeed(Math.abs(spr.speed), spr.direction)[0] * 0.5;
            }
            spr.onfloor = false;
        } else {
            spr.direction -= 90;
            ...
            if (spr.character.abilites.indexOf("spindash") > -1) {
                if (
                    ...
                    while (spr.down) {
                        ...
                        if (spr.character.abilites.indexOf("spindash") > -1) {
                            if (spr.jump) {
                                soundplay("spindash", spr.spindashpower / spr.speedcap + 1);
                                spr.spindashpower += 5;
...
function markSpritePos(spr, color) {
    collisionctx.fillStyle = color;
    collisionctx.fillRect(spr.x + (600 / 2) - 2, spr.y + (360 / 2) - 2, 4, 4);
}
async function lrcheck(spr, spr2, checkcollide, h) {
    ...
    var check = 0;
    var height = 24;
    var simplifidedDir = Math.round((scr_wrap_angle(spr.direction - 90) + 90) / 1) * 1;
    if (simplifidedDir !== 90) {
        height += 16;
    }
    var extend = spr.speedcap;
    spr.direction -= 90;
...
async function doFloorDirectionCheck(spr, spr2, checkcollide) {
    if (!spr.onfloor) {
        ...
        spr.angleDir = getAngleFromCollision(spr, spr2, checkcollide);
...
async function resetEngineValues(
    ...
    //this is for the ai, to know if we are an engine or not.
            ...
            if (!spr.inDebug) {
                ...
                if (true) {
                    if (!spr.dead) {
                                        ...
                                        if (spr.skidding) {
                                            ...
                                            if (spr.spindash) {
                                                ...
                                                if (spr.jumping || spr.rolling) {
                                                    ...
                                                    if (Math.abs(spr.savespeed * 5) > 4.5) {
                                                        spr.crouchTimer = 0;
                                                        spr.lookupTimer = 0;
                        ...
                        moveSteps(Math.round(spr.speed / 1) * 1, spr, checkcollide);
                        lrcheck(spr, spr2, checkcollide, 50);
                        if (spr.flying) {
                            spr.gravity +=  - (spr.gravityPower / 3);
                        } else {
                        ...
                        doFloorDirectionCheck(spr, spr2, checkcollide);
                        lrcheck(spr, spr2, checkcollide, 50);
                        var s = Math.round(fixDirAngle(scr_wrap_angle(spr.direction), 361) / 90) * 90;
                        var s2 = Math.round(fixDirAngle(scr_wrap_angle(spr.direction), 361) / 45) * 45;
                        spr.direction -= 90;
                        moveSteps(0, spr);
                        spr.direction += 90;
                        spr.direction -= 90;
                        ...
                        if (realAngle >= 135 && realAngle <= 225) {}
                            ...
                            if (spr.rolling) {
                                if (Math.sign(spr.speed) == Math.sign(Math.sin(spr.engineAngle - 90))) {
                        ...
                        if (spr.onfloor) {
                            if (spr.controlLockTimer == 0) {
                                ...
                                if ((Math.abs(spr.speed) < 2.5) && (x >= 35 && x <= 326) && (x >= 69 && x <= 293)) {
                                    // Detach (fall)
                                    spr.onfloor = false;
                                    ...
                                    spr.direction -= 90;
                                    moveSteps(spr.speedcap, spr); //turns out, speed cap is also used now for left and right checks.
                                    spr.direction += 90;
                                }
                            } else {
                                // Tick down timer
...
window.startEngine = async function startEngine(
    ...
    while (engineRunning) {
        ...
        for (var ts of window.files.level) {
            ...
            if (
                Math.abs(ts.x) > rangeOfOnScreenSprites.x + (ts.width * ts.scale) / 2 ||
